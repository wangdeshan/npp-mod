...
intptr_t CALLBACK AboutDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_INITDIALOG:
		{
            ...
            _pageLink.create(::GetDlgItem(_hSelf, IDC_HOME_ADDR), L"https://github.com/wangdeshan/npp-mod");
		...
		case WM_DRAWITEM:
		{
			...
			if (_hIcon == nullptr)
			{
				...
				// DPIManagerV2::loadIcon(_hInst, MAKEINTRESOURCE(NppDarkMode::isEnabled() ? IDI_TAIxxWANSSOVEREIGNTY_DM : IDI_TAIxxWANSSOVEREIGNTY), iconSize, iconSize, &_hIcon);
			}
